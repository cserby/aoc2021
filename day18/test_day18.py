...
from .day18 import (add_carry_to_the_leftmost, add_carry_to_the_rightmost,
                    add_snailfish_num, all_pairs, explode, magnitude,
                    parse_input, parse_snailfish_num, split, sum_snailfish_num)


def test_parse():
    assert parse_snailfish_num("[[[[1,3],[5,3]],[[1,3],[8,7]]],[[[4,9],[6,9]],[[8,2],[7,3]]]]") == (
        (((1, 3), (5, 3)), ((1, 3), (8, 7))), (((4, 9), (6, 9)), ((8, 2), (7, 3))))


def test_add():
    assert add_snailfish_num(parse_snailfish_num(
        "[1,2]"), parse_snailfish_num("[[3,4],5]")) == ((1, 2), ((3, 4), 5))
...
def test_sum():
    assert sum_snailfish_num(parse_input("day18/sample")) == parse_snailfish_num(
...
def test_all_pairs():
    assert set(all_pairs([1, 2, 3])) == {
        (1, 2), (1, 3), (2, 1), (2, 3), (3, 1), (3, 2)}


def test_part2():
    assert max(magnitude(add_snailfish_num(sn1, sn2))
               for sn1, sn2 in all_pairs(parse_input("day18/sample2"))) == 3993
